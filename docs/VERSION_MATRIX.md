...
| `V01.00.000-beta-01` | version single-sourcing + gate; `check --json` v0; coherent CLI (help/exit codes); SSE2 float arithmetic; fold-corruption + pinned-register fixes | unchanged | **internal allocator funnel**, poison-on-free debug | toml corrected | unchanged | unchanged | — | unchanged | — | — | CI secret-scan preflight | unchanged | superseded by beta-02 |
| `V01.00.000-beta-02` | `OMNI-E0005` honest backend/host refusal (`run`/`build` on non-x86-64, exit 2) | scalar kernel tier auto-selected off x86-64 | unchanged | unchanged | unchanged | unchanged | — | unchanged | — | **diagnostics tier live**: build + `check --json` + tiered suites on AArch64; arm64 CI lane | linux CI workflow YAML repaired (never parsed before) | unchanged | **current tag** |
