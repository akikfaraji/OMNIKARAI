...
| TD-20 | High | `src/codegen.c` (Win64 ABI paths) | First-ever executed Win64 JIT programs in CI (e0ad274+) reveal: pure computation passes (7/21 unit) but EVERY string or module call path SIGSEGVs (time/math/datetime/os/io/sys/list/ai + `sys.omni_ver` probe) | The legacy main target cannot run real programs; masked for years because windows.yml never reached an execution step (missing `make`, then assembler/linker errors, all fixed this iteration) | Debug the Win64 pinned-register + shadow-space + string-return sequence (RSI/RDI are callee-saved on Win64 but caller-saved on SysV — the pinning design differs per ABI and the Win64 side was never execution-tested) | V01.01 | DOCUMENTED (precisely scoped this iteration) |
